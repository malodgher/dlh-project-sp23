...
def init_params(options):
	...
	with open(options['embFile'], 'rb') as f:
		pm = pickle.load(f, encoding='latin-1')

	params['W_emb'] = np.array(pm).astype(config.floatX)
...
def load_data(seqFile, labelFile, timeFile=''):
	...
	if len(timeFile) > 0:
		with open(timeFile, 'rb') as f:
			times = pickle.load(f, encoding='latin-1')
...
def calculate_auc(test_model, datasets):
	...
	if len(unique_labels) == 1:
    	 print("ROC AUC score is not defined because only one class is present.")
	else:
...
def train_GRU_RNN(
	...
	for epoch in range(max_epochs):
		...
		if validAuc is not None:
			print('epoch:%d, valid_auc:%f' % (epoch, validAuc))
		else:
			print('Error calculating AUC score for epoch %d' % epoch)
...
if __name__ == '__main__':
	...
	train_GRU_RNN(dataFile=dataFile, labelFile=labelFile, timeFile=timeFile, embFile=embFile, 
		outFile=outFile, inputDimSize=inputDimSize, embDimSize=embDimSize, hiddenDimSize=hiddenDimSize, 
		max_epochs=max_epochs, L2_reg=L2_reg, batchSize=batchSize, use_dropout=use_dropout)
